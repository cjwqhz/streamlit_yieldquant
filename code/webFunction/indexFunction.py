...
def click_detect(img):
    opt = detect.my_parse_opt(sourcePath=img,weightPath="weights/best.pt")
...
def save_img(img_list):
    ...
    name = f'G:\\Python_project\\YOLO_DEPLOY\\yoloV5ToStreamlit-master\\picture\\image\\image_{now}.jpg'
...
def save_video(video):
    now = str(time.time()).split(".")[1]
    name = f'G:\\Python_project\\YOLO_DEPLOY\\yoloV5ToStreamlit-master\\picture\\video\\video_{now}.mp4'
